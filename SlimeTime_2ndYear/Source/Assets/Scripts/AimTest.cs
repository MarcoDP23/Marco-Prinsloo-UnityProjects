...
public class AimTest : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKey (RotateAntiClock) && GameControl.GetComponent<GameControl> ().TheGameState == global::GameControl.GameState.P1Shoot) {
			//Rotates the object AntiClockwise 
			angle = Mathf.Clamp(angle + (0.01f * Mathf.PI), 0, Mathf.PI) ;
			//Debug.Log (angle);

		} 
		else if (Input.GetKey (RotateClock) && GameControl.GetComponent<GameControl> ().TheGameState == global::GameControl.GameState.P1Shoot) {
	...
	void MovePointer () {//actually moves the psoition of the reticle
